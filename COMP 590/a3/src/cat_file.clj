      ...
      (let [contents (format-object directory database (first (rest args)))]
        (cond
          ...
          (let [tree-obj (unzip (str directory "/" database "/objects/" (subs (first (rest args)) 0 2) "/" (subs (first (rest args)) 2)))
                start (str/split (first (drop 1 (get (split-at-byte 0 contents) 0))) #"\s+")
                first-part (if (= "40000" (first start)) "040000" (first start))
                last-part (first (rest start))
                obj-type (if (= "class [B" (str (type tree-obj))) "blob" ())
                byte-stuff (first (rest (seq (split-at-byte 0 tree-obj))))]
            (println (str first-part " " obj-type " " (to-hex-string (take-last 20 byte-stuff)) "\t" last-part)))
