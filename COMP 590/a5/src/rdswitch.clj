(ns rdswitch
  (:require [clojure.java.io :as io]
            ...
            help init rev switch-branch explore))
      ...
      (let [directory (first (rest params))]
        (if (.exists (io/file directory))
          (cond
            ...
            (= (first (rest (rest params))) "rev-parse") (rev/rev-parse directory ".idiot" (rest (rest (rest params))))
            (= (first (rest (rest params))) "rev-list") (rev/rev-list directory ".idiot" (rest (rest (rest params))))
            (= (first (rest (rest params))) "explore") (explore/explore directory ".idiot" (rest (rest (rest params))))
            (= (first (rest (rest params))) "log") (rev/log directory ".idiot" (rest (rest (rest params))))
    ...
    (if (and (= switch1 "-d") (= nil switch2))
      (let [database (first (rest params))]
        (cond
          ...
          (= (first (rest (rest params))) "rev-parse") (rev/rev-parse "." database (rest (rest (rest params))))
          (= (first (rest (rest params))) "rev-list") (rev/rev-list "." database (rest (rest (rest params))))
          (= (first (rest (rest params))) "explore") (explore/explore "." database (rest (rest (rest params))))
          (= (first (rest (rest params))) "log") (rev/log "." database (rest (rest (rest params))))
    ...
    (if (or (and (= switch1 "-d") (= switch2 "-r")) (and (= switch1 "-r") (= switch2 "-d")))
      (let [database (if (= switch1 "-d") (first (rest params)) (nth params 3))
        ...
        (cond
          ...
          (= (first (rest (rest (rest (rest params))))) "rev-parse") (rev/rev-parse directory database (rest (rest (rest (rest (rest params))))))
          (= (first (rest (rest (rest (rest params))))) "rev-list") (rev/rev-list directory database (rest (rest (rest (rest (rest params))))))
          (= (first (rest (rest (rest (rest params))))) "log") (rev/log directory database (rest (rest (rest (rest (rest params))))))
          (= (first (rest (rest (rest (rest params))))) "explore") (explore/explore directory database (rest (rest (rest (rest (rest params))))))
