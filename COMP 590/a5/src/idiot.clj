(ns idiot
  (:require hash-object cat-file write-wtree commit-tree
            help rdswitch switch-branch rev init explore
...
(defn -main [& args]
  ...
  (if (empty? (rest args))
    (cond
      ...
      (= (first args) "commit") (println "Error: you must specify a tree address.")
      (= (first args) "rev-list") (rev/rev-list "." ".idiot" (rest args))
      (= (first args) "explore") (explore/explore "." ".idiot" (rest args))
      (= (first args) "log") (println "Error: log requires the --oneline switch")
  ...
  (if (or (empty? args) (str/includes? args "-r") (str/includes? args "-d") (str/includes? args "-h") (str/includes? args "--help") (str/includes? args "help"))
    ...
    (if (seq (rest args))
      (cond
        ...
        (= (first args) "rev-parse") (rev/rev-parse "." ".idiot" (rest args))
        (= (first args) "rev-list") (rev/rev-list "." ".idiot" (rest args))
        (= (first args) "log") (rev/log "." ".idiot" (rest args))
        (= (first args) "explor") (explore/explore "." ".idiot" (rest args))
